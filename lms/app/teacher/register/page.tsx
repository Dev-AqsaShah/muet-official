...
const SPECIALIZATIONS = [
  'Web Development', 'Python & Data Science', 'Database Management',
  'Graphic Designing & UI/UX', 'Digital Marketing & SEO', 'Mobile App Development',
  'Cyber Security', 'Cloud Computing', 'Freelancing & E-Commerce',
]

export default function TeacherRegisterPage() {
  const router = useRouter()
  const [form, setForm] = useState({
    fullName: '', email: '', mobile: '', specialization: '', facultyCode: '', password: '',
  })
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 py-10" style={{ background: '#020B18' }}>
      ...
      <div className="relative z-10 w-full max-w-md">
        <Link href="/teacher/login" className="inline-flex items-center gap-1.5 text-xs mb-4" style={{ color: '#607896' }}>
          <ArrowLeft size={13} /> Teacher login
        </Link>
        ...
        <div className="rounded-2xl p-8" style={{ background: 'rgba(6,18,36,0.92)', border: `1px solid ${ACCENT}26` }}>
          <div className="flex flex-col items-center mb-7">
            ...
            <p className="text-xs text-center" style={{ color: '#607896' }}>
              For MUET instructors — requires the faculty code issued by administration
            </p>
          </div>
          ...
          {done ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm font-semibold" style={{ color: '#E8F4FF' }}>Account created!</p>
              <p className="text-xs" style={{ color: '#607896' }}>Redirecting you to teacher login…</p>
            </div>
          ) : (
            <>
              {error && (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#607896' }}>Full Name</label>
                  <input type="text" value={form.fullName} onChange={set('fullName')}
                    placeholder="Engr. Your Name" required style={inputStyle} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#607896' }}>Email Address</label>
                  <input type="email" value={form.email} onChange={set('email')}
                    placeholder="you@muet.edu.pk" required style={inputStyle} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#607896' }}>Mobile Number</label>
                  <input type="text" value={form.mobile} onChange={set('mobile')}
                    placeholder="03XXXXXXXXX" required style={inputStyle} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#607896' }}>Specialization</label>
                  <select value={form.specialization} onChange={set('specialization')} required
                    style={{ ...inputStyle, appearance: 'none' as const }}>
                    <option value="" disabled>Select your subject area</option>
                    {SPECIALIZATIONS.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#607896' }}>Faculty Registration Code</label>
                  <input type="text" value={form.facultyCode} onChange={set('facultyCode')}
                    placeholder="Issued by MUET administration" required style={inputStyle} />
                </div>
                <div>
                ...
                <button type="submit" disabled={loading}
                  ...
                  style={{
                    background: loading ? `${ACCENT}33` : `linear-gradient(135deg, ${ACCENT}, #00E5C8)`,
                    color: loading ? '#607896' : '#020B18',
                    cursor: loading ? 'not-allowed' : 'pointer',
                  }}>
                  {loading
                    ? <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                    : <><UserPlus size={16} /> Create Faculty Account</>}
                </button>
              </form>

              <p className="text-center text-xs mt-6" style={{ color: '#607896' }}>
                Already registered? <Link href="/teacher/login" style={{ color: ACCENT }}>Sign In</Link>
              </p>
            </>
          )}
        </div>
