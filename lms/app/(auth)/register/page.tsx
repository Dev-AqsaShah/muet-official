...
const PROGRAMMES = ['PITP','BBSHRRDB','NFTP'] as const
const COURSES = {
  PITP:     ['Web Development','Graphic Designing & UI/UX','Digital Marketing & SEO','Python Development','Data Science','Cyber Security','Mobile App Development','E-Commerce'],
  BBSHRRDB: ['Web Development','Graphic Designing','Digital Marketing','Python Development','Technical Freelancing'],
  NFTP:     ['Technical Track','Non-Technical Track','Creative Design & Freelancing'],
}
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    district: '', qualification: '', programme: '', course: '',
    password: '', confirm: '',
  })
  ...
  const canNext2 = form.district && form.qualification && form.programme && form.course
          ...
          <div className="p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="date" value={form.dob} onChange={set('dob')} style={inputStyle}
                      onFocus={e => (e.target.style.borderColor='#00E5C8')} onBlur={e => (e.target.style.borderColor='rgba(0,229,200,0.15)')} />
                    {form.dob && !ageOk() && <p className="text-xs mt-1" style={{ color: '#EF4444' }}>Age must be 28 or below to be eligible.</p>}
                  </div>
                  <div>
                    {label('Gender')}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div>
                  {label('Programme')}
                  <select value={form.programme} onChange={e => setForm(f => ({ ...f, programme: e.target.value, course: '' }))} style={inputStyle}
                    onFocus={e => (e.target.style.borderColor='#00E5C8')} onBlur={e => (e.target.style.borderColor='rgba(0,229,200,0.15)')}>
                ...
                {form.programme && (
                  <div>
                    ...
                    <select value={form.course} onChange={set('course')} style={inputStyle}
                      ...
                      {COURSES[form.programme as keyof typeof COURSES].map(c => <option key={c}>{c}</option>)}
                    </select>
                  </div>
                )}
              </div>
            ...
            <p className="text-center text-xs mt-4" style={{ color: '#607896' }}>
              Already registered? <Link href="/login" style={{ color: '#00E5C8' }}>Sign In</Link>
            </p>
          </div>
        </div>
      </div>
